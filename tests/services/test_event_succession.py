...
async def preserved(
...
) -> tuple[Webinar | Slot, RetainedEventRight, str]:
    ...
    if payout is not None:
        (required(await db.get(BookingPayment, id=payment_id))).payout_coins = payout
    ...
    return event, required(right), required(payment_id)


async def authorize(
    mocker: MockerFixture, right: RetainedEventRight, subject: str = THIRD
) -> tuple[str, dict[str, Any], AsyncMock]:
    grant_id = str(uuid4())
    authority = {
        "id": grant_id,
        "source": "events",
...
async def test_existing_event_delivery_preserves_original_payment_period_and_role(
...
async def test_t2_withdraws_exact_grant_then_new_election_continues_same_seat(
...
async def test_changed_admission_and_erased_target_do_not_grant(
    session: AsyncSession, remote: dict[str, Any], mocker: MockerFixture
) -> None:
    ...
    assert (required(await db.get(EventSubjectGuard, subject=THIRD))).deleted is True and right.current_subject is None


async def test_existing_other_seat_is_not_merged(
    session: AsyncSession, remote: dict[str, Any], mocker: MockerFixture
) -> None:
...
async def test_ended_period_requires_resolution_without_extension(
    session: AsyncSession, remote: dict[str, Any], mocker: MockerFixture
) -> None:
    event, right, _ = await preserved(remote)
...
async def test_provider_continuation_covers_existing_participants_without_reopening_sales(
    session: AsyncSession, remote: dict[str, Any], mocker: MockerFixture
) -> None:
    ...
    assert await existing_events(THIRD, event.id) == []
    assert (required(await db.get(BookingPayment, id=first.payment_id))).user_id == OTHER


@pytest.mark.parametrize("kind", ["webinar", "coaching"])
async def test_cancel_succeeded_participation_preserves_original_financial_owner(
    ...
    assert claims[0].coins == (required(await db.get(BookingPayment, id=payment_id))).paid_coins
...
async def test_cleanup_succeeded_host_retains_original_remuneration_owner(
    session: AsyncSession, remote: dict[str, Any], mocker: MockerFixture, payout: bool
) -> None:
    from api.models import SettlementClaim
    ...
    assert (required(await db.get(BookingPayment, id=payment_id))).user_id == OTHER
