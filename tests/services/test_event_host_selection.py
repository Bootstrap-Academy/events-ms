...
from api.models import BookingContract, BookingPayment, EventRightGrant, RetainedEventRight, WebinarParticipant
...
async def unavailable(right, state):
...
async def test_only_exact_elected_host_grant_controls_link(session, remote, mocker, unready_index, state):
    event, rights = await two_rights(remote)
    original_owners = {r.payment_id: (await db.get(BookingPayment, id=r.payment_id)).user_id for r in rights}
    ...
    assert {r.payment_id: (await db.get(BookingPayment, id=r.payment_id)).user_id for r in rights} == original_owners
    remote[THIRD] = canonical(THIRD)
    await delete_user_data(THIRD)
    assert await existing_events(THIRD, event.id) == []
    assert (await db.get(EventRightGrant, id=grant)).state == "withdrawn"


async def test_second_exact_grant_can_remain_usable_after_first_requires_resolution(session, remote, mocker):
    event, rights = await two_rights(remote)
    for right in rights:
...
async def test_ready_sibling_without_election_cannot_replace_unavailable_exact_grant(session, remote, mocker):
    event, rights = await two_rights(remote)
...
async def test_all_scoped_webinar_views_use_exact_host_election(session, remote, mocker, elected_index, state, route):
    ...
    original = {r.payment_id: (await db.get(BookingPayment, id=r.payment_id)).user_id for r in rights}

    async def assert_links(expected):
    ...
    assert len(await db.all(filter_by(WebinarParticipant, webinar_id=event.id))) == 2
    assert {r.payment_id: (await db.get(BookingPayment, id=r.payment_id)).user_id for r in rights} == original
    assert (await db.get(EventRightGrant, id=grant_id)).state == ("withdrawn" if state == "withdrawn" else "granted")


@pytest.mark.parametrize("state", ["review", "closed", "withdrawn", "before_window"])
async def test_scoped_coaching_calendar_uses_retained_host_readiness_and_window(session, remote, mocker, state):
    ...
    remote[USER] = canonical(USER)
    await delete_user_data(USER)
    right = await retained_events.right_for(event.payment_id, "instructor")
...
async def test_mounted_scoped_host_reads_agree_after_exact_contract_review(session, remote, mocker, client):
