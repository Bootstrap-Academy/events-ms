...
from tests.endpoints.test_webinars import (
    STUDENT,
    _user,
    _webinar,
    clear_cache_patch,
...
async def test_scoped_webinar_offer_acceptance_uses_same_exact_contract(session, spend_coins):
    event = _webinar()
    event.id = str(uuid4())
    await db.add(event)
    ...
    contract = await db.get(BookingContract, id=payments[0].id)
    assert contract.acceptance == acceptance.payload() and contract.state == "ready"
    assert spend_coins.await_count == 1
    guard = await db.get(EventSubjectGuard, subject=STUDENT)
    ...
    assert (await export_user_data(STUDENT)).retained_event_rights["booking_reservations"] == guard.booking_reservations
    assert (await export_user_data(str(uuid4()))).retained_event_rights["booking_reservations"] == {}


async def test_erased_subject_cannot_place_a_new_booking_from_an_old_offer(session, spend_coins):
    event = _webinar()
    event.id = str(uuid4())
    await db.add(event)
    offered = await learning.webinar_offer(UUID(event.id), _user(STUDENT))
    ...
    (await db.get(EventSubjectGuard, subject=STUDENT)).deleted = True
    await db.commit()
...
async def test_wrong_exact_offer_still_has_no_booking_or_debit(session, spend_coins):
    event = _webinar()
    event.id = str(uuid4())
    await db.add(event)
    offered = await learning.webinar_offer(UUID(event.id), _user(STUDENT))
...
async def test_scoped_route_mounting_body_and_path_use_existing_booking_pipeline(session, client, spend_coins):
...
async def test_old_coaching_booking_header_cannot_claim_unrelated_rebooking(
    session, mocker, spend_coins, first_outcome
):
    from datetime import timedelta
    from unittest.mock import AsyncMock

    from api.endpoints.calendar import _try_cancel_coaching
    ...
    for path in (
        ...
        "api.endpoints.calendar.clear_cache",
    ...
    slot = await Slot.create(host, utcnow() + timedelta(days=9), utcnow() + timedelta(days=9, hours=1))
    await db.add(Coaching(user_id=host, skill_id="test", price=800))

    async def book(subject):
    ...
    if first_outcome == "rejected":
        ...
        assert (await db.get(BookingPayment, id=first)).state == "failed"
        spend_coins.return_value = "paid"
    else:
        first = await book(STUDENT)
        batch = await settlements.new_batch("cancellation", STUDENT, slot.id)
        assert await _try_cancel_coaching(slot.id, _user(STUDENT), batch)
        await db.commit()
        assert (await db.get(BookingContract, id=first)).closed is True
    ...
    assert (await db.get(EventSubjectGuard, subject=STUDENT)).booking_reservations[first]["event_id"] == slot.id
...
async def test_scoped_new_participant_reads_keep_confirmed_booking_access(session, mocker, spend_coins):
    from datetime import timedelta
    from unittest.mock import AsyncMock
