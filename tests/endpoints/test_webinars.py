...
async def test__register_for_webinar__records_what_was_paid(session: AsyncSession, spend_coins: AsyncMock) -> None:
    ...
    await register_for_webinar(webinar, _user(STUDENT))

    assert spend_coins.await_args_list == [call(STUDENT, PRICE, "Webinar 'test webinar'")]
...
async def test__register_for_webinar__not_enough_coins(session: AsyncSession, spend_coins: AsyncMock) -> None:
    spend_coins.return_value = False
    webinar = await db.add(_webinar())
...
async def test__register_for_webinar__mails_the_amount_that_was_paid(
    ...
    recipient, title, body = send_email.await_args_list[0].args[:3]
    assert (recipient, title) == (f"{STUDENT}@example.com", "Anmeldungsbestätigung - Bootstrap Academy")
    assert "Preis: 0 MorphCoins" in " ".join(body.split())


async def test__register_for_webinar__failing_mail__books_anyway(
    session: AsyncSession, send_email: AsyncMock, spend_coins: AsyncMock
) -> None:
    """The coins are already gone when the mail is sent, so a mail that fails must not fail the booking."""

    send_email.side_effect = ValueError("Invalid email address")
    webinar = await db.add(_webinar())

    result = await register_for_webinar(webinar, _user(STUDENT))
