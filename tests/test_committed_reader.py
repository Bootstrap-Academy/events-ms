...
async def test_reader_reserves_existing_budget_with_no_reader_overflow(mocker, size, overflow, general):
    create = mocker.patch("api.database.database.create_async_engine")
    value = DB("synthetic", reserve_committed_reader=True, pool_size=size, max_overflow=overflow)
...
async def test_impossible_or_unbounded_budget_is_not_silently_increased(mocker, size, overflow):
    create = mocker.patch("api.database.database.create_async_engine")
...
async def test_both_pools_are_disposed(mocker):
